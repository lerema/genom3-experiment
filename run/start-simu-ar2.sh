...
tmux \
    ...
    split-window -p 80 "arucotag-pocolibs -i arucotag2 -f |& tee -i $logdir/arucotag.log" \; \
    split-window -p 80 "$user_cmd" \; \
    split-window -h  -t 0 "camgazebo-pocolibs -i camgazebo2 -f |& tee -i $logdir/arucotag.log" \; \
    split-window -h -t 2 "camviz-pocolibs -i camviz2 -f |& tee -i $logdir/arucotag.log" \; \
    selectp -t 4
